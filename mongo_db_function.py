...
client = pymongo.MongoClient("mongodb+srv://s50302911:jasons10730149@lineapp.0s2ex81.mongodb.net/?retryWrites=true&w=majority")
...
def store_game(user_id):
    game.insert_one({"User_id": user_id, "answer": "waiting"})

def find_game(user_id):
    return game.find_one({"User_id":user_id})

def update_game(user_id, computeranswer):
    game.update_one({"User_id":user_id}, {"$set": {"answer": computeranswer}})
    
def delete_game(user_id):
    game.delete_one({"User_id": user_id})

def store_profile(dict):
    profile.insert_one(dict)

def find_profile(user_id):
    return profile.find_one({"User_id":user_id})

def delete_profile(user_id):
    profile.delete_one({"User_id":user_id})
    
def update_status(user_id, status):
    profile.update_one({"User_id": user_id}, {"$set":{"status":status}}) 

def check_profile_exist(user_id):
    if find_profile(user_id) is None:
        return False
    else:
        return True

def check_game_exist(user_id):
    if find_game(user_id) is None:
        return False
    else:
        return True
